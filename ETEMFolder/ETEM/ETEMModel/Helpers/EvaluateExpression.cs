...
namespace ETEMModel.Helpers
{
    ...
    {
        private int COUNT_PRIMATIVES = 10;
        private Dictionary<string, string> Primitives;
        ...
        public EvaluateExpressionHelper()
        {
            Primitives = new Dictionary<string, string>();
        }

        public Decimal EvalExpression(string expression, Dictionary<string, string> primitives)
        {
            // Store the primitives.
            Primitives = new Dictionary<string, string>();

            if (primitives.Count > COUNT_PRIMATIVES)
            {

                throw new FormatException("The primitives should be less than " + COUNT_PRIMATIVES);
            }
        ...
        private Decimal EvaluateExpression(string expression)
        {
                ...
                switch (expr.Substring(best_pos, 1))
                {
                    case "^":
                        return Convert.ToDecimal( Math.Pow(
                            Convert.ToDouble(EvaluateExpression(lexpr)),
                            Convert.ToDouble(EvaluateExpression(rexpr))));
                    case "*":
                        return
            ...
            if (expr_len > 5 && expr.EndsWith(")"))
            {
                ...
                if (paren_pos > 0)
                {
                    ...
                    {
                        case "sin":
                            return Convert.ToDecimal( Math.Sin(Convert.ToDouble(EvaluateExpression(rexpr))) );
                        case "cos":
                            return Convert.ToDecimal(Math.Cos(Convert.ToDouble(EvaluateExpression(rexpr))));
                        case "tan":
                            return Convert.ToDecimal( Math.Tan(Convert.ToDouble(EvaluateExpression(rexpr))));
                        case "sqrt":
                            return Convert.ToDecimal(Math.Sqrt(Convert.ToDouble(EvaluateExpression(rexpr))));
                        case "factorial":
                            return Convert.ToDecimal(Factorial(Convert.ToDouble(EvaluateExpression(rexpr))));
            ...
            if (Primitives.ContainsKey(expr))
            {
                ...
                try
                {
                    ...
                    //return double.Parse(Primitives[expr]);

                    return BaseHelper.ConvertToDecimalOrZero(Primitives[expr].ToString());
                }
                catch (Exception)
                {
                    throw new FormatException(
            ...
            try
            {
                ...
                return BaseHelper.ConvertToDecimalOrZero(expr.ToString());
            }
            catch (Exception)
            {
                throw new FormatException(
