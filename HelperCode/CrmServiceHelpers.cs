        ...
        public virtual Configuration GetServerConfiguration()
        {
            ...
            if (addConfig)
            {
                ...
                if (config.ServerAddress.EndsWith(".dynamics.com", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    {
                    config.DiscoveryUri =
                        new Uri(String.Format("http://disco.{0}/XRMServices/2011/Discovery.svc", config.ServerAddress));
                    }
                    else
                    {
                    config.DiscoveryUri =
                        new Uri(String.Format("http://dev.{0}/XRMServices/2011/Discovery.svc", config.ServerAddress));
